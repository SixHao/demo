            ...
            <div class="row-content am-cf">
                    ...
                    <div class="am-u-sm-12 am-u-md-12 am-u-lg-12">
                        <div class="widget am-cf">
                            ...
                            <div class="widget-body am-fr">
                                ...
                                <form class="am-form tpl-form-border-form tpl-form-border-br" action="{{ url('/admin/cate/update/'.$cates['tid']) }}" method="post" enctype="multipart/form-data">
                                  {{ csrf_field() }}
              ...
              @endif
                                    ...
                                    <div class="am-form-group">
                                        <label for="user-name" class="am-u-sm-3 am-form-label" value="{{$cates['pid']}}">父级分类</label>
                                        <div class="am-u-sm-9">
                               <select name="pid"  style="background-color:#4b5357">
                               <option>--请选择--</option>
                            @foreach($data as $k=>$v)
                            @if($v->pid == '0')
                            <option value="{{ $v['tid'] }}" {{ $cates['pid'] == $v['tid'] ? 'selected' : '111' }} >{{ $v['tname'] }}</option>
                          @else
                          <option value="{{ $v['tid'] }}" {{ $cates['pid'] == $v['tid'] ? 'selected' : '111' }} >{{str_repeat('&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;',$v->lev)}}{{ $v['tname'] }}</option>
                          @endif
                            @endforeach
                        </select>
